"""
NNQS Phase 3: 6D Coupled Harmonic Oscillator

System:
    V(x) = 1/2 * sum(x_i^2) + 0.15 * sum(x_i * x_{i+1})  (periodic)
    E_exact = 2.982753 (from normal mode analysis)

This is the direct comparison with LP+PINN, which achieved:
    - Best: 2.9885 (0.19% error) at hour 8
    - Final: 2.996 (0.45% error) at hour 20
    - Training time: 20.2 hours
"""
...
def coupled_potential_6d(x, coupling=0.15):
    """
    6D coupled harmonic oscillator with periodic nearest-neighbor coupling.

    V(x) = 1/2 * sum(x_i^2) + lambda * sum(x_i * x_{i+1})
    where x_7 = x_1 (periodic boundary).
    """
    # Diagonal terms: 0.5 * sum(x_i^2)
    V = 0.5 * torch.sum(x ** 2, dim=-1)

    # Coupling terms: lambda * sum(x_i * x_{i+1}) with periodic boundary
    for i in range(6):
        j = (i + 1) % 6
...
def main():
    print("=" * 70)
    print("NNQS Phase 3: 6D Coupled Harmonic Oscillator")
    print("V(x) = 0.5*sum(x_i^2) + 0.15*sum(x_i*x_{i+1})")
    print("E_exact = 2.982753 (normal mode analysis)")
    print("=" * 70)
    ...
    E_loc_init = compute_local_energy(model, test_x, lambda x: coupled_potential_6d(x, COUPLING))
    ...
    print(f"\nLP+PINN comparison: Best 0.19% at 8h, Final 0.45% at 20.2h")
    print(f"\n{'Epoch':>6} {'Energy':>10} {'E_std':>8} {'Accept':>8} {'Error%':>8} {'Best%':>8} {'Time':>8}")
    print("-" * 70)
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        # Compute local energy with coupled potential
        E_loc = compute_local_energy(model, x, lambda x: coupled_potential_6d(x, COUPLING))
        f = model(x)
        ...
        if epoch % 100 == 0:
            ...
            print(f"{epoch:6d} {E_val:10.4f} {E_std.item():8.4f} "
                  f"{sampler.acceptance_rate:7.2%} {error_pct:8.2f}% {best_error:8.2f}% {mins:7.1f}m")
        ...
        if epoch > 0 and epoch % 200 == 0:
            ...
            if avg_accept < target_accept_low * 0.8:
                ...
                print(f"  [Step size -> {sampler.step_size:.3f} (accept {avg_accept:.1%} < {target_accept_low:.0%})]")
            elif avg_accept > target_accept_high * 1.2:
                sampler.step_size *= 1.15
                print(f"  [Step size -> {sampler.step_size:.3f} (accept {avg_accept:.1%} > {target_accept_high:.0%})]")
        ...
        if epoch > 0 and epoch % CHECKPOINT_INTERVAL == 0:
            ...
            torch.save(checkpoint, 'nnqs_6d_coupled_checkpoint.pt')
    ...
    def test_acceptance_coupled(walkers, step_size, n_steps=100):
    ...
    best_ss = 0.5
    best_diff = 1.0
    for ss in [0.2, 0.3, 0.4, 0.5, 0.7, 1.0]:
        acc, _ = test_acceptance_coupled(fresh_walkers, ss)
        diff = abs(acc - 0.4)
    ...
    _, walkers = test_acceptance_coupled(fresh_walkers, best_ss, n_steps=500)
    ...
    all_x_sq = []
    for i in range(15):
        acc, walkers = test_acceptance_coupled(walkers, best_ss, n_steps=100)
        w = walkers.clone().requires_grad_(True)
        E_loc = compute_local_energy(model, w, lambda x: coupled_potential_6d(x, COUPLING))
    ...
    fresh_x_sq_mean = np.mean(all_x_sq)

    print(f"\n| Metric                  | Measured   | Expected   |")
    print(f"|-------------------------|------------|------------|")
    print(f"| E_loc mean              | {fresh_E_mean:10.4f} | {E_EXACT:10.4f} |")
    print(f"| E_loc std               | {fresh_E_std:10.4f} |    <0.5    |")
    print(f"| |x|^2 mean              | {fresh_x_sq_mean:10.4f} |   ~3.0     |")

    fresh_error = abs(fresh_E_mean - E_EXACT) / E_EXACT * 100
    print(f"\nFresh walker energy error: {fresh_error:.2f}%")

    # ============================================================
    # Comparison with LP+PINN
    # ============================================================
    print("\n" + "-" * 70)
    print("Comparison with LP+PINN")
    print("-" * 70)

    print(f"\n| Method   | Best Error | Final Error | Time to Best | Total Time |")
    print(f"|----------|------------|-------------|--------------|------------|")
    print(f"| LP+PINN  |     0.19%  |      0.45%  |     8 hours  | 20.2 hours |")
    print(f"| NNQS     | {best_error:8.2f}%  | {final_error:10.2f}%  | {best_epoch/60:10.1f} min | {total_time/60:8.1f} min |")
    ...
    ax1.axhline(E_EXACT, color='r', linestyle='--', linewidth=2, label=f'Exact: {E_EXACT:.4f}')
    ...
    ax2.axhline(0.45, color='orange', linestyle=':', label='LP+PINN final (0.45%)')
    ax2.axhline(0.19, color='purple', linestyle=':', label='LP+PINN best (0.19%)')
    ...
    plt.suptitle(f'NNQS 6D Coupled Oscillator: E = {final_E:.4f} ({final_error:.2f}% error)\n'
                 f'Best: {best_energy:.4f} ({best_error:.2f}%) | Training: {total_time/60:.1f} min',
                 fontsize=12, fontweight='bold')
    plt.tight_layout()
    plt.savefig('nnqs_6d_coupled_convergence.png', dpi=150, bbox_inches='tight')
    print("Saved: nnqs_6d_coupled_convergence.png")
    ...
    if success:
        print(f"\nSUCCESS: NNQS achieved {best_error:.2f}% error on 6D coupled oscillator!")
        if best_error < 0.45:
            print("NNQS OUTPERFORMED LP+PINN in accuracy!")
        if total_time < 20.2 * 3600:
            speedup = (20.2 * 3600) / total_time
            print(f"NNQS was {speedup:.0f}x FASTER than LP+PINN!")
    else:
        print(f"\nDid not reach 1% error. Best: {best_error:.2f}%")
        print("May need more training time or hyperparameter tuning.")
