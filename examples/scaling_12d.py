"""
Dimensional Scaling Study Phase 2a: 12D Henon-Heiles All-to-All
...
def run_single_trial(seed, dim, max_epochs=10000, max_time=45*60, verbose=True):
    ...
    # Network - scale hidden size with dimension
    hidden_size = 512 + 64 * (dim - 6)  # 512 for 6D, 896 for 12D
...
def main():
    DIM = 12
    E_HARMONIC = DIM / 2.0  # 6.0 for 12D

    print("=" * 70)
    print(f"Dimensional Scaling Study Phase 2a: {DIM}D Henon-Heiles All-to-All")
    print("=" * 70)
    ...
    cv_ok = cv_E < 1.0
    eloc_ok = np.mean(final_stds) < 1.5  # Slightly relaxed for higher D
    scaling_ok = 0.45 < E_per_dim < 0.55
    fresh_ok = abs(mean_fresh - mean_E) / mean_E * 100 < 1.0  # Slightly relaxed

    print(f"\n1. Self-consistency (CV < 1%):     {'PASS' if cv_ok else 'FAIL'} (CV = {cv_E:.4f}%)")
    print(f"2. E_loc variance (< 1.5):         {'PASS' if eloc_ok else 'FAIL'} (std = {np.mean(final_stds):.4f})")
